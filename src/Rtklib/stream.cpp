...
*           2019/10/21 1.26 add api for vrs connector,modified by juntao at wuhan university
*-----------------------------------------------------------------------------*/
#ifndef _WIN32
#include <fcntl.h>
#include <unistd.h>
...
#define MAXCLI              256         /* max client connection for tcp svr */
...
#define MSG_LOOP            262144   /* self defined*/
#define MSG_NOTUSE          -1       /* self defined*/       
/* macros --------------------------------------------------------------------*/
...
/* type definition -----------------------------------------------------------*/
	...
	typedef struct {
		...
		vrsauth_i_t con[MAXCLI]; /* authorized clients */
	}vrscon_svr_t;
	...
	typedef struct {            /* ntrip caster connection type */
		...
		int msgid; /* distributed id for client */
	} ntripc_con_t;
	...
	static serial_t *openserial(const char *path, int mode, char *msg)
	{
		...
		char parity = 'N', dev[128], port[128], fctr[64] = "", path_tcp[32], msg_tcp[128];
		const char* p;
#ifdef WIN32
	...
	static int send_block(socket_t sock,unsigned char* buff,int n){
		int ns;
	...
	static ntripc_t *openntripc(const char *path, int type, char *msg)
	{
		...
		if (!(ntripc->tcp = opentcpsvr(tpath, msg))) {
			tracet(1, "openntripc: opentcpsvr error port=%d\n", port);
			free(ntripc);
			return NULL;
		}
		return ntripc;
	...
	static void rsp_ntripc_c(ntripc_t *ntripc, int i)
	{
		...
		if (con->nb >= NTRIP_MAXRSP - 1) { /* buffer overflow */
			tracet(1, "rsp_ntripc_c: request buffer overflow\n");
		...
		/* test mountpoint */
		//if (!*mntpnt || (!test_mntpnt(ntripc, mntpnt) && !Deploy::s_getInstance()->m_checkIsVrs(mntpnt))) {
		...
		/* test vrs */
		//bvrs = Deploy::s_getInstance()->m_checkIsVrs(mntpnt);
		/* send OK response */
		send_nb(ntripc->tcp->cli[i].sock, (unsigned char *)rsp2, strlen(rsp2));
		con->state = 1;
		strcpy(con->mntpnt, mntpnt);
		/* self defined code */
		con->msgid = MSG_NOTUSE;
		con->nb = 0;
		if (bvrs)
			con->msgid = ++currentMsgCount%MSG_LOOP;
	}
	...
	static int readntripc(ntripc_t *ntripc, unsigned char *buff, int n, char *msg)
	{
		...
		for (i = 0; i < MAXCLI; i++) {
			...
			if (nr < 0) {
				...
				/* record msgid,self defined code */
				ntripc->msgid = ntripc->con[i].msgid;
				return nr;
			}
		}
		return 0;
	...
	static int writentripc(ntripc_t *ntripc, unsigned char *buff, int n, char *msg)
	{
		...
		for (i = 0; i < MAXCLI; i++) {
			...
			/* skip if not the selected client,self defined code*/
			if (ntripc->msgid != MSG_NOTUSE && ntripc->msgid != ntripc->con[i].msgid) {
				continue;
			}
	...
	/* vrs connector client */
	static vrscon_cli_t* openvrsconcli(const char* path, char* msg) {
		vrscon_cli_t* vrscon_cli;
		char addr[256] = "", port[256] = "", tpath[MAXSTRPATH];
		if (!(vrscon_cli = (vrscon_cli_t*)calloc(1, sizeof(vrscon_cli_t)))) return NULL;
		vrscon_cli->state = 0;
		vrscon_cli->nb = 0;
		decodetcppath(path, addr, port, vrscon_cli->user, vrscon_cli->passwd, NULL, NULL);
		sprintf(tpath, "%s:%s", addr, port);
		if (!(vrscon_cli->tcp = opentcpcli(tpath, msg))) {
			tracet(1, "openntrip: opentcp error\n");
			free(vrscon_cli);
			return NULL;
		}
	...
	static void closevrsconcli(vrscon_cli_t* vrscon) {
		closetcpcli(vrscon->tcp);
		free(vrscon);
	}
	static int reqvrscon_c(vrscon_cli_t* vrscon_cli, char* msg) {
//		VrsMsgConnector req;
//		sprintf(req.usrpsd, "%s:%s", vrscon_cli->user, vrscon_cli->passwd);
//		VrsEncoder::s_getInstance()->IOEncode(&req, 100);
//		if (writetcpcli(vrscon_cli->tcp, (unsigned char*)req.buff, req.nbyte, msg)!= req.nbyte) return 0;
//		vrscon_cli->state = 1;
		return 1;
	}
	static int rspvrscon_c(vrscon_cli_t* vrscon_cli, char* msg) {
		int i,ret,j;
		char *p, *q;
//		VrsMsgConnector req;
//		if (vrscon_cli->nb >= sizeof(vrscon_cli->buff) - 1) {
//			vrscon_cli->nb = 0;
//			vrscon_cli->buff[0] = '\0';
//			vrscon_cli->state = 0;
//			tracet(2, "rspvrscon_c:buff overflow failed");
//			discontcp(&vrscon_cli->tcp->svr, vrscon_cli->tcp->tirecon);
//			return 0;
//		}
//		for (i = 0; i < vrscon_cli->nb; i++) {
//			ret = VrsEncoder::s_getInstance()->IODecode(&req, vrscon_cli->buff[i]);
//			if (ret == 103) {
//				q = (char*)vrscon_cli->buff;
//				p = (char*)vrscon_cli->buff + i + 1;
//				vrscon_cli->nb = vrscon_cli->nb - i - 1;
//				for (j = 0; j < vrscon_cli->nb; j++) *q++ = *p++;
//				vrscon_cli->state = 2;
//				tracet(2,"rspvrscon_c:response ok nb = %d",vrscon_cli->nb);
//				return 1;
//			}
//			else if (ret == 102) {
//				vrscon_cli->nb = 0;
//				vrscon_cli->buff[0] = '\0';
//				vrscon_cli->state = 0;
//				tracet(2, "rspvrscon_c:authorize failed");
//				discontcp(&vrscon_cli->tcp->svr, vrscon_cli->tcp->tirecon);
//			}
//		}
		return 0;
	}
	static int waitvrsconcli(vrscon_cli_t* vrscon_cli, char* msg) {
		int n;
		char *p;
		if (vrscon_cli->state < 0) return 0;
		if (vrscon_cli->tcp->svr.state < 2) vrscon_cli->state = 0; /* tcp disconnected */
		if (vrscon_cli->state == 0) { /* send request */
			if (!reqvrscon_c(vrscon_cli, msg)) return 0;
		}
		if (vrscon_cli->state == 1) { /* read response */
			p = (char*)vrscon_cli->buff + vrscon_cli->nb;
			if ((n = readtcpcli(vrscon_cli->tcp, (unsigned char *)p, sizeof(vrscon_cli->buff) - vrscon_cli->nb - 1, msg)) == 0) {
				tracet(5, "waitntrip: readtcp n=%d\n", n);
				return 0;
			}
			vrscon_cli->nb += n; vrscon_cli->buff[vrscon_cli->nb] = '\0';
			return rspvrscon_c(vrscon_cli,msg);
		}
		return 1;
	}
	static int readvrsconcli(vrscon_cli_t* vrscon_cli, unsigned char* buff, int n, char* msg) {
		int nb,i;
		char*p, *q;
		if (!waitvrsconcli(vrscon_cli, msg)) return 0;
		if (vrscon_cli->nb > 0) { /* read response buffer first */
			nb = vrscon_cli->nb <= n ? vrscon_cli->nb : n;
			memcpy(buff, vrscon_cli->buff + vrscon_cli->nb - nb, nb);
			vrscon_cli->nb = vrscon_cli->nb - nb;
			p = (char*)vrscon_cli->buff;
			q = (char*)vrscon_cli->buff + nb;
			for (i = 0; i < vrscon_cli->nb; i++) *p++ = *q++;
			return nb;
		}
		return readtcpcli(vrscon_cli->tcp, buff, n, msg);
	}
	static int writevrsconcli(vrscon_cli_t* vrscon_cli, unsigned char* buff, int n, char* msg) {
		if (!waitvrsconcli(vrscon_cli, msg)) return 0;
		return writetcpcli(vrscon_cli->tcp, buff, n, msg);
	}
	/* vrs connector server */
	static vrscon_svr_t* openvrsconsvr(const char* path, char* msg) {
		vrscon_svr_t *vrscon_svr;
//		int i, j;
//		char port[256] = "", tpath[MAXSTRPATH] = { 0 };
//		if (!(vrscon_svr = (vrscon_svr_t *)malloc(sizeof(vrscon_svr_t)))) return NULL;
//		vrscon_svr->state = 0;
//		vrscon_svr->user[0] = vrscon_svr->passwd[0] = '\0';
//		for (i = 0; i < MAXCLI; i++) {
//			vrscon_svr->con[i].state = 0;
//			vrscon_svr->con[i].nb = 0;
//			for (j = 0; j < sizeof(vrscon_svr->con[i].buff); j++) vrscon_svr->con[i].buff[j] = 0;
//		}
//		/* decode tcp/ntrip path */
//		decodetcppath(path, NULL, port, vrscon_svr->user, vrscon_svr->passwd, NULL, NULL);
//		memset(tpath, 0, sizeof(tpath));
//		sprintf(tpath, "%s:%s", vrscon_svr->user, vrscon_svr->passwd);
//		VrsEncoder::s_getInstance()->enbase64(vrscon_svr->m_ptrAuth, (unsigned char*)tpath, strlen(tpath));
//
//		memset(tpath, 0, sizeof(tpath));
//		sprintf(tpath, ":%s", port);
//		/* open tcp server stream */
//		if (!(vrscon_svr->tcp = opentcpsvr(tpath, msg))) {
//			tracet(1, "openntripc: opentcpsvr error port=%d\n", port);
//			free(vrscon_svr);
//			return NULL;
//		}
	...
	static void closevrsconsvr(vrscon_svr_t* vrscon_svr) {
		closetcpsvr(vrscon_svr->tcp);
		free(vrscon_svr);
	}
	static void discon_vrsconsvr(vrscon_svr_t* vrscon_svr, int i) {
		discontcp(&vrscon_svr->tcp->cli[i], ticonnect);
		vrscon_svr->con[i].nb = 0;
		vrscon_svr->con[i].buff[0] = '\0';
		vrscon_svr->con[i].state = 0;
	}
	static void rsp_vrscon_s(vrscon_svr_t* vrscon_svr, int i) {
		/* receive corresponding auth*/
//		int ib,ret,j;
//		VrsMsgConnector req;
//		vrsauth_i_t* con = &vrscon_svr->con[i];
//		con->buff[con->nb] = '\0';
//
//		if (con->nb >= sizeof(con->buff) - 1) {
//			tracet(1, "rsp_vrscon_s,request buffer overflow\n");
//			discon_vrsconsvr(vrscon_svr, i);
//			return;
//		}
//		for (ib = 0; ib < con->nb; ib++) {
//			ret = VrsEncoder::s_getInstance()->IODecode(&req, con->buff[ib]);
//			if (ret == 100) { /* authorize success  */
//				if (!strncmp(vrscon_svr->m_ptrAuth, req.authbuf, strlen(vrscon_svr->m_ptrAuth))) {
//					for (j = 0; j < MAXCLI; j++) { /* disconnect others */
//						if (vrscon_svr->con[i].state && j != i)
//							discon_vrsconsvr(vrscon_svr, j);
//					}
//					cout << "authorized success for " << vrscon_svr->tcp->cli[i].sock << endl;
//					con->state = 1;
//				}
//				VrsEncoder::s_getInstance()->IOEncode(&req, con->state ? 103 : 102);
//				send_nb(vrscon_svr->tcp->cli[i].sock, (unsigned char*)req.buff, req.nbyte);
//				break;
//			}
//		}
	}
	static void waitvrsconsvr(vrscon_svr_t* vrscon_svr, char* msg) {
		/* wait for authentication */
		unsigned char *buff;
		int i, n, nmax, err;
		tracet(4, "wait_ntripc\n");
		vrscon_svr->state = vrscon_svr->tcp->svr.state;
		if (!waittcpsvr(vrscon_svr->tcp, msg)) return;
		for (i = 0; i < MAXCLI; i++) {
			if (vrscon_svr->tcp->cli[i].state != 2 || vrscon_svr->con[i].state) continue;
			/* receive ntrip-caster request */
			buff = vrscon_svr->con[i].buff + vrscon_svr->con[i].nb;
			nmax = sizeof(vrscon_svr->con[i].buff) - vrscon_svr->con[i].nb - 1;

			if ((n = recv_nb(vrscon_svr->tcp->cli[i].sock, buff, nmax)) == -1) {
				if ((err = errsock())) {
					tracet(1, "accept_ntripc: recv error sock=%d err=%d\n",
						vrscon_svr->tcp->cli[i].sock, err);
				}
				discon_vrsconsvr(vrscon_svr, i);
				continue;
			}
			...
			/* test ntrip-caster request */
			vrscon_svr->con[i].nb += n;
			rsp_vrscon_s(vrscon_svr,i);
		}
	}
	
	static int readvrsconsvr(vrscon_svr_t* vrscon_svr, unsigned char* buff, int n, char* msg) {
		int i, nr, err;
		waitvrsconsvr(vrscon_svr, msg);
		for (i = 0; i < MAXCLI; i++) {
			if (!vrscon_svr->con[i].state) continue;
			nr = recv_nb(vrscon_svr->tcp->cli[i].sock, buff, n);
			if (nr < 0) {
				if ((err = errsock())) {
					tracet(1, "readntripc: recv error i=%d sock=%d err=%d\n", i,
						vrscon_svr->tcp->cli[i].sock, err);
				...
				vrscon_svr->tcp->cli[i].tact = tickget();
				return nr;
			}
		}
		return 0;
	}
	static int writevrsconsvr(vrscon_svr_t* vrscon_svr, unsigned char* buff, int n, char* msg) {
		int i, ns = 0, err,nsd;
		waitvrsconsvr(vrscon_svr, msg);
		for (i = 0; i < MAXCLI; i++) {
			if (!vrscon_svr->con[i].state) continue;
			nsd = 0;
			while(nsd < n){
				if((ns = send_block(vrscon_svr->tcp->cli[i].sock, buff + nsd, n - nsd)) == -1){
					if ((err = errsock())) {
						tracet(1, "writentripc: send error i=%d sock=%d err=%d\n", i,
							vrscon_svr->tcp->cli[i].sock, err);
					}
...
#endif
	{
		...
		char cmd[2048], env[1024] = "", opt[1024];
		const char *proxyopt = "";
		const char *proto;
		int ret;
	...
	*-----------------------------------------------------------------------------*/
	extern void strclose(stream_t *stream)
	{
		...
		if (stream->port) {
			...
			case STR_VRSCONCLI: closevrsconcli((vrscon_cli_t*)stream->port); break;
			case STR_VRSCONSVR: closevrsconsvr((vrscon_svr_t*)stream->port); break;
			}
		}
		else {
	...
	*-----------------------------------------------------------------------------*/
	extern int strread(stream_t *stream, unsigned char *buff, int n)
	{
		...
		case STR_VRSCONCLI: nr = readvrsconcli((vrscon_cli_t*)stream->port, buff, n, msg); break;
		case STR_VRSCONSVR: nr = readvrsconsvr((vrscon_svr_t*)stream->port, buff, n, msg); break;
		default:
			strunlock(stream);
			return 0;
		}
	...
	*-----------------------------------------------------------------------------*/
	extern int strwrite(stream_t *stream, unsigned char *buff, int n)
	{
		...
		case STR_VRSCONCLI: ns = writevrsconcli((vrscon_cli_t*)stream->port, buff, n, msg); break;
		case STR_VRSCONSVR: ns = writevrsconsvr((vrscon_svr_t*)stream->port, buff, n, msg); break;
		case STR_FTP:
		case STR_HTTP:
	...
	*-----------------------------------------------------------------------------*/
	...
	extern int strgetmsgid(stream_t *stream, int *msgid) {
		if (!stream->port ||
			(stream->type != STR_NTRIPC_S && stream->type != STR_NTRIPC_C)) {
			return 0;
		}
		strlock(stream);
		*msgid = ((ntripc_t*)stream->port)->msgid;
		strunlock(stream);
		return 1;
	}
	extern int strsetmsgid(stream_t *stream, int msgid) {
		if (!stream->port ||
			(stream->type != STR_NTRIPC_S && stream->type != STR_NTRIPC_C)) {
			return 0;
		}
		...
		((ntripc_t*)stream->port)->msgid = msgid;
		strunlock(stream);
		return 1;
	}
	/* get stream status -----------------------------------------------------------
	...
	*-----------------------------------------------------------------------------*/
	...
	extern bool validMsg(int msgid) {
		return msgid != MSG_NOTUSE;
	}
	}
