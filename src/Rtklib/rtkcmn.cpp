...
static char codepris[7][MAXRTKFREQ][16]={  /* code priority table */
...
* args   : int    sat       I   satellite number (1-MAXRTKSAT)
...
extern void setcodepri(int sys, int freq, const char *pri)
{
    ...
    if (freq<=0||MAXRTKFREQ<freq) return;
...
* return : a'*b
*-----------------------------------------------------------------------------*/
extern double rtk_dot(const double *a, const double *b, int n)
{
    double c=0.0;
...
{
    double e2=FE_WGS84*(2.0-FE_WGS84),r2=rtk_dot(r,r,2),z,zk,v=RE_WGS84,sinp;
...
extern void uniqnav(nav_t *nav)
{
    ...
    for (i=0;i<MAXRTKSAT;i++) for (j=0;j<NFREQ;j++) {
...
extern int expath(const char *path, char *paths[], int nmax)
{
    ...
    char p_path[1024] = {0};
    strcpy(p_path,path);
    if ((p=strrchr(p_path,'/'))||(p=strrchr(p_path,'\\'))) {
        file=p+1; strncpy(dir,p_path,p-p_path+1); dir[p-p_path+1]='\0';
    }
...
extern double satazel(const double *pos, const double *e, double *azel)
{
    ...
    if (pos[2]>-RE_WGS84) {
        ecef2enu(pos,e,enu);
        az=rtk_dot(enu,enu,2)<1E-12?0.0:atan2(enu[0],enu[1]);
        if (az<0.0) az+=2*pi;
        el=asin(enu[2]);
    }
...
{
    double H[4*MAXRTKSAT],Q[16],cosel,sinel;
    int i,n;
    
    for (i=0;i<4;i++) dop[i]=0.0;
    for (i=n=0;i<ns&&i<MAXRTKSAT;i++) {
                     ...
                     int opt, double *dant)
{
    ...
    for (i=0;i<NFREQ;i++) {
        ...
        dant[i]=-rtk_dot(off,e,3)+(opt?interpvar(90.0-azel[1]*R2D,pcv->var[i]):0.0);
    }
...
{
    double Ps[2][MAXRTKSAT][NFREQ]={{{0}}},Lp[2][MAXRTKSAT][NFREQ]={{{0}}},dcp;
    int i,j,s,r,n[2][MAXRTKSAT][NFREQ]={{{0}}};
    obsd_t *p;
    ...
    for (i=0;i<obs->n;i++) {
        ...
        for (j=0;j<NFREQ;j++) {
            if (s<=0||MAXRTKSAT<s||r<=0||2<r) continue;
